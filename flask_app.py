...
v2v_model = KeyedVectors.load('180/small.kv')
all_words = df_nouns["word_0"]
    ...
    for sample in combinations(green_ai, groupsize):
        ...
        for cand, _ in candidates:  # cand у меня вида "слово_NOUN"
            ...
            if cand.split("_")[0] in session.get(
                ...
                continue
            if (
                cand.split("_")[0] in session.get('all_hints_backend')
...
def play():
    ...
    all_words = df_nouns["word_0"]
    print(len([w for w in all_words if w in v2v_model]))
    wordlist = random.sample([w for w in all_words if f'{w}_NOUN' in v2v_model], 25)
    ...
    session['all_hints_frontend'] = []
    session['all_hints_backend'] = []
